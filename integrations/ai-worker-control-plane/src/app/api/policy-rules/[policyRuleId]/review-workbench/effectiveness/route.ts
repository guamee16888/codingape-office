...
export async function GET(
  ...
  try {
    ...
    return Response.json({
      ...
      policy_review_work_item_effectiveness: listPolicyReviewWorkItemEffectiveness(policyRuleId, {
...
export async function POST(
  ...
  try {
    ...
    return Response.json({ policy_review_work_item_effectiveness: result }, { status: 201 });
  } catch (error) {
    if (error instanceof AuthError) {
      return Response.json({ error: error.message }, { status: error.status });
    }
